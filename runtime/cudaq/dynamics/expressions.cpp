...
//     auto func = [&](std::map<int,int> none, std::map<std::string,
...
//       for (std::size_t i = 0; i + 1 < degree; i++) {
//         mat(i + 1, i) = 0.5 * std::sqrt(static_cast<double>(i + 1)) + 0.0 *
//         'j'; mat(i, i + 1) = 0.5 * std::sqrt(static_cast<double>(i + 1)) +
//         0.0 * 'j';
//       }
//       std::cout << "dumping the complex mat: \n";
//       mat.dump();
...
//       annihilate) ] for (std::size_t i = 0; i + 1 < degree; i++) {
//         temp_mat(i + 1, i) =
...
complex_matrix ElementaryOperator::to_matrix(
  ...
  std::variant<complex_matrix, std::complex<double>> result =
      m_ops[id].m_generator(degrees, parameters);
