...
private:
  ...
  std::function<std::complex<double>(const std::string&, const std::complex<double>&)> _value_function;
  int _current_idx;
...
public:
  ...
  // Postfix increment.
  /// @brief Postfix increment operator to move to the next step in the schedule.
  ...
  /// @return The current complex step value as an optional. If no valid step, returns std::nullopt.
