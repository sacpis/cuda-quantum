...
uniform_superposition = numpy.ones(
    2**num_qubits, dtype=numpy.complex128) / numpy.sqrt(2**num_qubits)
all_zero_state = numpy.array([1] + (2**num_qubits - 1) * [0],
...
schedule.reset()
evolution_result = evolve_async(hamiltonian, dimensions, schedule,
                                uniform_superposition)
...
start_time, end_time = 0, 1
steps = numpy.linspace(start_time, end_time, num_steps)
schedule = Schedule(steps, ["t"], lambda param, step: step
                    if param == "t" else 0.0)

# Define Scalar and Elementary Operators
so1 = ScalarOperator(lambda t: t)
elop1 = pauli.x(0)
elop2 = pauli.z(1)

# Define a ProductOperator as a combination of Scalar and Elementary Operators
product_op = ProductOperator([so1, elop1, elop2])

# Define an OperatorSum containing the ProductOperator
operator_sum = OperatorSum([product_op])

# Define an ArithmeticOperation that will be evaluated
arithmetic_op = ArithmeticOperation([[product_op]])

# Evaluate using evaluate_schedule method
result = operator_sum.evaluate_schedule(schedule, dims)

# Test if the results are as expected
print("Testing evaluate_schedule:")
for i, step_result in enumerate(result):
    print(f"Step {i} (t={steps[i]}):")
    print(step_result)
