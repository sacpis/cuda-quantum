...
def _evolution_kernel(num_qubits: int,
                      compute_step_matrix: Callable[[Mapping[
                          str, NumericType]], NDArray[numpy.complexfloating]],
...
def evolve(
    ...
    observable_spinops = (
        lambda step_parameters: op._to_spinop(dimensions, **step_parameters)
        for op in observables)
    compute_step_matrix = lambda step_parameters: _compute_step_matrix(
        hamiltonian, dimensions, step_parameters, True)

    # FIXME: deal with a sequence of initial states
    if store_intermediate_results:
        evolution = _evolution_kernel(num_qubits,
                                      compute_step_matrix,
                                      parameters,
        ...
        kernel = next(
            _evolution_kernel(num_qubits, compute_step_matrix, parameters))
        if len(observables) == 0:
